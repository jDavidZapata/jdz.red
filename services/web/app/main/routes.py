...
from app.models import User, Post, Message, Notification, Vote
...
def index():
    ...
    names = (
        db.session
        .query(Vote.suggested_name, func.count().label('count'))
        .group_by(Vote.suggested_name)
        .order_by(func.count().desc())
        .all()
    )

    if request.method == "POST":
        suggested_name = (request.form.get('name') or '').capitalize()
        error = None

        if not current_user.is_authenticated:
            error = "Must Login Or Register To Make A Suggestion!!!"
        ...
        if error:
            ...
            try:
                ...
                count = db.session.query(Vote).filter_by(author=current_user).count()
    
                if count >= 5:
                    flash('Your already casted 5 votes!!!')
                    return redirect(url_for("main.index"))
                
                vote = Vote(suggested_name=suggested_name, author=current_user)
                db.session.add(vote)
                db.session.commit()
                ...
                current_app.logger.error(f"Error adding vote: {e}")
                flash('An error occurred while processing your suggestion. Please try again.')
